  ...
  $scope.addAddress = function () {
    if(checkForm()){
        ...
        $http.post(`/rest/address/form`, item).then((resp) => {
            // resp.data should contain id and detail fields
            const addr = resp.data;
            // Append to existing select and select it
            const $select = $("select[name='address_id']");
            if ($select.length) {
                const text = $scope.form.detail || (addr.detail || "Địa chỉ mới");
                const value = addr.id;
                // remove placeholder option if exists
                $select.find("option:contains('Vui lòng thêm')").remove();
            ...
            // Switch back to existing address and go next step
            $("input[name='payment_address'][value='existing']").prop('checked', true).trigger('change');
            transferMethod();
        });
    }
  }
